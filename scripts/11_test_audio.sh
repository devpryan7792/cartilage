...
LOG_FILE="${BUILD_DIR}/11_test_audio.log"

TOTAL_START=$SECONDS

echo "============================================================"
echo "Cartilage OS — Audio Subsystem Verification Suite (Task 12)"
echo "Target Cartridge: ${CARTRIDGE_IMG}"
echo "Kernel:           ${KERNEL}"
echo "============================================================"

# Step 1: Ensure cartridge is built with audio subsystem
...
# Step 2: Boot QEMU with intel-hda sound device and run audio test hook
echo ""
echo "============================================================"
echo "==> Step 2: Booting QEMU with intel-hda and running audio test..."
echo "============================================================"
mkdir -p "${BUILD_DIR}"
...
timeout 75s qemu-system-x86_64 \
  ...
  -append "console=ttyS0 root=/dev/vda rootfstype=erofs init=/init cartilage_test_audio=1" \
  -display none \
  -serial stdio \
...
echo ""
echo "============================================================"
echo "Cartilage OS — Task 12 Verification Assertions"
echo "============================================================"
if grep -q "\[PASS\] Audio subsystem verification successful" "${LOG_FILE}"; then
    TOTAL_ELAPSED=$(( SECONDS - TOTAL_START ))
    echo "  [PASS] Cartridge Build:                   ${T_BUILD_ELAPSED}s"
    echo "  [PASS] QEMU Audio Test:                   ${T_QEMU_ELAPSED}s"
    echo "  TOTAL EXECUTION TIME:                     ${TOTAL_ELAPSED}s"
    echo "============================================================"
    echo "==> [PASS] Task 12 Checkpoint Passed: Audio subsystem verification successful!"
    exit 0
else
    echo "==> [FAIL] Expected '[PASS] Audio subsystem verification successful' not found in test log." >&2
    echo "--- Log output ---" >&2
    cat "${LOG_FILE}" >&2
    exit 1
fi
