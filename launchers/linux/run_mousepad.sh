...
CARTRIDGE_IMG="${BUILD_DIR}/cartridge_mousepad.img"

if [[ ! -f "${CARTRIDGE_IMG}" ]]; then
    if [[ -f "${BUILD_DIR}/cartridge_mousepad_arch.img" ]]; then
        CARTRIDGE_IMG="${BUILD_DIR}/cartridge_mousepad_arch.img"
    else
        echo "Error: Mousepad cartridge image not found in ${BUILD_DIR}." >&2
        exit 1
    fi
fi
...
echo "============================================================"
echo "Starting Cartilage OS — Text Editor (Mousepad - Arch glibc)"
