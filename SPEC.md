...
## Task 11 — Network & DNS Pipeline

**Done when:**
- `/init` automatically brings up the primary Ethernet interface and requests an IP lease via DHCP without starting NetworkManager or systemd-networkd.
- `/etc/resolv.conf` resolves external hostnames inside the booted cartridge namespace.
- Running `curl -I https://1.1.1.1` and `getent hosts archlinux.org` inside a booted QEMU cartridge returns exit code 0.

**Test command:**
```bash
bash scripts/10_test_networking.sh
```
Pass = HTTP response code 200/301 from external host and valid DNS lookup. Fail = `Network is unreachable` or resolution failure.

---

## Task 12 — Audio Subsystem

**Done when:**
- Unprivileged user `cartilage` (`UID 1000`) has access to `/dev/snd/*` without root permission errors.
- ALSA software mixing (`dmix`) is configured via `/etc/asound.conf` with zero background daemons.
- Sound test generates clean audio output without blocking or crashing.

**Test command:**
```bash
bash scripts/11_test_audio.sh
```
Pass = ALSA device opens cleanly and returns 0 without permission errors.

---

## Task 13 — Modern Web Kiosk Runtime (Chromium)

**Done when:**
- `build_cartridge.sh --app chromium` compiles an immutable EROFS cartridge with native Wayland Ozone support.
- Booting the Chromium cartridge in QEMU launches Chromium fullscreen under `cage` without crashing.
- Chromium's internal zygote sandbox initializes cleanly without user namespace collisions.

**Test command:**
```bash
bash scripts/12_test_chromium.sh
```
Pass = Chromium window appears in `cage`, renders test page, exits cleanly without zygote panic.

---

## Task 14 — Bare-Metal USB Flasher Script

**Done when:**
- `scripts/13_flash_usb.sh` safely targets a specified block device, refusing fixed internal drives unless `--force-internal` is set.
- Generates a valid GPT partition table containing ESP (FAT32), cartridge partitions (EROFS), and `CARTDATA` (ext4/exFAT).
- Populates the fallback UEFI bootloader path `\EFI\BOOT\BOOTX64.EFI`.
- QEMU booting the flashed drive image boots into `systemd-boot` using PARTLABEL routing.

**Test command:**
```bash
bash scripts/13_test_flasher.sh
```
Pass = Flashed disk image boots successfully with PARTLABEL root routing.

---

## Task 15 — Alpine Lightweight Runtime

**Done when:**
- `build_cartridge.sh --runtime alpine --app mousepad` uses `apk.static` to construct a minimal `musl`-based rootfs.
- The resulting `.img` file is smaller than 50MB.
- Boots into `cage` running the target GUI application cleanly.

**Test command:**
```bash
bash scripts/14_test_alpine_cartridge.sh
```
Pass = Output `.img` < 50MB and successfully executes app in QEMU.

---

# Phase 3: The Cartilage Appliance Framework ("The Bigger Shift")

Phase 3 transitions Cartilage OS from an exploratory prototype into an engineered, declarative appliance compiler and universal bare-metal engine.

---

## Task 16 — Declarative Appliance Specification (`cartilage.yaml`)

**Done when:**
- `spec/cartilage.schema.json` is created, defining a strict JSON Schema for appliance manifests.
- Manifest validates required sections: `appliance` (name, version), `runtime` (engine: alpine|arch, packages), `display` (compositor, mode: desktop|kiosk, entrypoint, args), `storage` (mode: ephemeral|persistent|host-access, quota), and `hardware` (network, audio, acceleration).
- Invalid manifests produce clean, actionable error messages pinpointing exact line and key errors.

**Test command:**
```bash
python3 -c "import json, jsonschema; print('Schema is valid JSON')"
```
Pass = Manifest validator rejects invalid configs and passes standard recipes.

---

## Task 17 — The Unified `cartilage` CLI Engine

**Done when:**
- A single executable Python CLI `cartilage` is implemented in `src/cartilage/` with zero third-party pip dependencies (standard library only).
- Provides subcommands:
  - `cartilage validate <recipe.yaml>`
  - `cartilage build <recipe.yaml> [-o output.img]`
  - `cartilage run <recipe.yaml|image.img> [--url URL]`
  - `cartilage compose -o <combined.img> <recipe1.yaml> <recipe2.yaml>`
  - `cartilage flash --target <block-device> <recipe1.yaml> ...`
- `cartilage run` automatically maps hardware flags (KVM, memory, virtio-net, virtio-gpu, sound) directly from the recipe manifest, completely eliminating duplicated launcher scripts.

**Test command:**
```bash
python3 -m cartilage --help
```
Pass = CLI executes cleanly with full command help and accurate subcommands.

---

## Task 18 — Modular `/init.d/` Stage Runner

**Done when:**
- The monolithic 600-line `/init` heredoc in `build_cartridge.sh` is extracted and replaced by structured stage scripts under `/init.d/`:
  - `00-vfs.sh`: Kernel virtual filesystems (`/proc`, `/sys`, `/dev`, `/run`, `/tmp`).
  - `10-hardware.sh`: Device hotplugging (`udevadm`/`mdev`), DRM nodes, input devices, kernel module loading.
  - `20-network.sh`: Ethernet/Wi-Fi interface detection, non-blocking DHCP lease, Anycast DNS.
  - `30-storage.sh`: Storage policy enforcement (ephemeral tmpfs overlay vs. persistent ext4).
  - `40-security.sh`: Namespace unsharing, binary masking, dropping root privileges to user `cartilage` (UID 1000).
  - `50-launch.sh`: `seatd` daemon launch, Wayland compositor (`cage`), and appliance execution.
- Every stage contains explicit error boundaries: if `/data` is read-only or hardware is missing, init logs a warning and degrades gracefully (e.g. falling back to in-memory overlayfs) instead of killing PID 1 and triggering a kernel panic.

**Test command:**
```bash
cartilage run build/cartridge_dillo_arch.img --test
```
Pass = VM boots through all 6 stages sequentially with clean logs and no panic.

---

## Task 19 — Standard Recipe Hub (`recipes/`)

**Done when:**
- Standard, verified recipe manifests are authored and tested under `recipes/`:
  - `recipes/browser-chromium.yaml` — Modern Chromium web browser with tabs, omnibox, and DuckDuckGo start page.
  - `recipes/browser-dillo.yaml` — Ultra-fast, lightweight Xwayland web browser.
  - `recipes/editor-mousepad.yaml` — Focused text editor workstation.
  - `recipes/terminal-foot.yaml` — Minimalist Wayland terminal station.
- `cartilage build recipes/<name>.yaml` compiles each recipe into a bootable EROFS cartridge with no manual intervention.

**Test command:**
```bash
cartilage validate recipes/*.yaml
```
Pass = All 4 standard recipes validate against schema and build bootable cartridges.

---

## Task 20 — Universal Bare-Metal Portability & USB Flash Engine

**Done when:**
- The graphics subsystem auto-detects physical GPU devices (Intel `i915`, AMD `amdgpu`, VirtIO) and automatically falls back to software rendering (`pixman`/`llvmpipe`) if hardware acceleration is unavailable.
- `cartilage flash --target /dev/sdX` safely inspects the target device, rejects mounted host root drives, writes the GPT partition layout (ESP, cartridges, `CARTDATA`), installs UEFI fallback loaders, and produces a live bootable USB stick.
- Booted on a physical x86_64 PC, Cartilage launches directly from USB to the appliance GUI in under 2.5 seconds.

**Test command:**
```bash
cartilage flash --dry-run /dev/null recipes/browser-dillo.yaml
```
Pass = Flash dry-run completes with partition layout calculation and zero errors.

---

## General rule across all tasks

A task is marked complete only when its "Done when" criteria above are met and the listed test command has actually been run and its output checked — not when the code "should" work. If a test can't be run for some reason, say so explicitly next to the checkbox instead of marking it done.
